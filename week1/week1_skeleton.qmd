...
balance_summary %>% 
    # Use as_gt() when coming from a summary table
    as_gt() %>% 
 ...
 tab_header(
  ...
  # Add a Spanner to group the data columns
    tab_spanner(
        ...
        # Breaker line under Randomized Groups
...
```{r}
tbl_2 %>% 
    # Use `gt()` when coming from a dataframe
    gt(rowname_col = "year") %>% 
    tab_header(
...
tbl_3 %>% 
    gt(rowname_col = "year") %>% 
    tab_header(
        title = "European Lobster Catch by Treatment, Region, and Year",
...
table_w_plots <- lobsters %>% 
    ...
    group_by(year) %>% 
    summarize(total_cpue = sum(cpue, na.rm = TRUE),
              # Store all individual cpue values as a list for each year
              dist_cpue = list(cpue),
              .groups = "drop"
              ) %>% 
    arrange(year) %>% 
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "blowfish"); beep(9)  
...
Air quality issues, waste burning
2. How were the treatment and control groups formed? 
44 nieghborhoods, 
3. Was the social competition strategy effective? 
Yes, it was 24% reduction

5. the metric they use were the number of waste piles
