...
author: "EDS 241 / ESM 244, Filled in: Marie"
format:
  html:
    theme: sketchy
    css: styles.css
...
*Response:* _Reef_Exposure: Exposed and Pollution Risk: Low and dist_mangroves
...
*Response:* For mangrove alot of small fishes grow there, a shorter distance might increase the number of fish. For reef exposure, More exposed reefs are exposed to waves and nutrients
...
*Response:* The average increase in fish. The population and inference are now different, the inference is now the type of MPA. Not a difference in size.
...
*Response:* 108 treated were matched, 37 unique control sites were matched

**Q6.** Find the covariate with the largest standardized mean difference (SMD) *after matching*. Which covariate is it?
# Compare before and after. In general look at after. 
# In `dist_to_deep_water` it goes worse because we are trying to match 10 covariates at the same time, optimizing balance on some of the controls it made balance worse on the other ones
*Response:* dist_to_deep_water, dist_mangroves, 

Visualize balance on the covariates using `plot()` with `type = "density"`. *To see the effectiveness of matching*

```{r}
plot(match_model, type = "density", interactive = FALSE,
     which.xs = ~dist_to_deep_water + ssta_freq + reef_exposure)
# dist_to_deep_water after matched it is more imbalance
```
...
love.plot(
    ...
    # Using new column names made
    var.names = nice_names
)

# Straight solid line is a perfect match
```
...
```{r}
# Biomass_fisheries is a column in matched data
...
*Response:* __Ecological: On average treated effect MPA site has 79.50 kg/hectare more in the OLS linear regression after matching.

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* Unobserved covariate variables, we don't know if we got enough observations. Selection bias.
...
- `plot` fixed effects absorb all time-invariant differences across plots (e.g., baseline soil quality), *155 plots*
- `year` fixed effects absorb common shocks shared by all plots in a given year (e.g., a region-wide drought year), *two years*

```{r}
fe_lm <- lm(perinc ~ vpd + I(vpd^2) + dbh + density +
                # Using as dummy variable
                plot + year,
             data = data_fe)
...
*Response:* The vpd got doubled in size from 0.126 to 0.227, the treatment estimates changed if there is a bias within the treatment effect. 
nofe:0.126
felm: 0.227
...
*Response:* Confounding variables (year) can differ from year to year from weather events. 
...
*Response:* On a plot it can be soil, nutrients, and proximity near to streams.
