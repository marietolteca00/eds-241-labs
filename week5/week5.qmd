...
- The running variable ($X$): A continuous measure used for assignment: *Distance to measured area*

- The Cutoff ($c$): The threshold rule: *Boundary*

- Treatment rule ($D$): What changes at the cutoff (policy / intervention): *Protect or Non-Protected Area*
...
rdd_lm <- ggplot(data_binned, aes(x = avg_distance, y = avg_deforest, color = as.factor(protected)))+
    ...
    labs(
        title = "Regression Discontinuity Plot (Binned by Distance)",
...
- The unprotected area has 
the trends reveal the the further it is away from zero in protected the deforestation rate decreases. In unprotected areasthe closer it is from 0, the faster the rate drops.
...
- The comparison would between protected and unprotected area would be 
...
rdd_ols <- lm(
    deforest ~ # Outcome
        ...
        slope_cat + road_cat + water_cat + soil_cat, # Control Variables
    data = sim_data
)
...
**RESPONSE**:
Casual effect -  Average 10% decrease deforestation in protected areas

Does it match what you see in the restuls - it represents the drop of (-0.1)

interaction coefficient - protected:distance (0.002) represents the slope of protected areas compared to the slope in unprotected areas
...
- *Takes more parameters than OLS*
...
- A simulation of bandwidth optimization far smaller spread of data, we are cutting observation. We are specifying a cutoff and a kernel whereas in the previous regression table we did not specify. Another difference would be the weights, we are valuing more.
- We are optimizing the bandwidth, a non-linear, to make it more accustomed to our data. This is taking more into account than the ols.
...
**RESPONSE**:
The rdd_ploy (plot on the right) captured the data better, the points follow the line closer to the regression line. Much steeper to the cutoff line, in terms of how the lines are non-linear.
...
**RESPONSE**:
- Comparing our final plot with neal 2024, there is a similar comparison considering ours is simulated data. There are some discrepencies.
