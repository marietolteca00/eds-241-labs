...
weekly_la_phx <- did_data_az_ca %>% 
    mutate(
        date_obj = mdy(date),
        # Assinging a week for each observation starts on Monday
        week = floor_date(date_obj, unit = "week")
    ) %>% 
    filter(year == 2020, month(date_obj)
...
ggplot(data = weekly_la_phx, aes(x  = week, y = mean_no2, color = city)) +
    geom_line(size = 1.2) +
    geom_point(size = 2.5) +
...
**Your answer here.** The parallel trends for `DiD` are violating the assumption as they are two very different cities. Another thing to point out was that in the graph, there is a big spike before the lockdown begins.
...
**Your answer here.** The trends are similar, but not the best. 
...
did_data_nm_tx <- did_data_nm_tx %>% 
    mutate(
        date_obj = mdy(date),
        # Dummy Variable = Treatment: Treat = NM & Control = TX
        treated  = ifelse(state == "New Mexico", 1, 0),
        
        # Dummy Variable: Post = After NM lockdown
...
*The difference in difference only interpret the `treated:is_post`
:::{.callout-tip title="Comprehension Check"}
# control is texas
# treated is new mexico
What does this treatment effect of -3.516 mean? Talk with a partner! 
- The lockdown is `-3.516` relatively lower than texas 
:::
...
**Yes, Texas makes a compelling counterfactual that satisfies the assumption for DiD. The trend fluctuation are more difficult to predict, however they are still a good parallel trend to compare with each other.  **
...
**The coefficient (`treated:is_post`) is not statistically significant with a p-value at 0.22. The last row does not have an asterix, there is 22% probability that we would observe this to happen with no strict lockdown.**
...
**The counterfactual was texas, el paso. They had similar characteristics and bandwidth for state borderlines, similar weather, etc. If a dust storm affect both states, it would no ruin the DiD estimates since both states would be hit with a storm not altering resutls.**
...
**The DiD estimate would be smaller, if both groups stayed home.**