...
Good pre-treatment fit is the key credibility condition.If we cannot match trends before treatment, we cannot trust the counter-factual after. When evaluating the pre-treatment fit, consider how many measurement time points are available, the volatility of the trend, and the magnitude of deviations between observed and synthetic California.
...
-   Different size in population meaning that they are not starting even. We would not be able to trust the counter factual.

-   Different covariates that might influences results. Not an apple to apples comparison.
...
2.  What does it mean if the synthetic control closely tracks the treated unit in the pre-treatment period? What if it doesn't?
    -   Track closely, if it doesn't it would mean it is not a good match, could not interpret causal effect. Cannot compare before or after treatment, with differences in pre-treatment
...
-   This would be make sure CA has similar CO2 emissions post-treatment, when using the \``` co2_power_pc_tons` ``

-   Covariates also need to match
...
1.  How many states are in the donor pool? How many years does the data cover?
    -   49 Total States (length(unique(data\$state)))
...
scm_data <- data %>%
  synthetic_control(
    # Outcome Variable: per capita CO2
    outcome = co2_power_pc_tons,
    # Unit Identifier(STATE)
    unit = state,
    # Year Identifier(YEAR)
    time = year,
    # Intervention (treated) unit: CA
    i_unit = "CA",
    # Intervention (treatment) year: 2013
    i_time = 2013,
    
    
    # Generating Placebo Tests for ALL states
    generate_placebos = TRUE) %>%
    
    # Defining Characteristics Synthetic California must match in order to be considered a good copy of real CA
  generate_predictor( 
      
    # Pre-Treat Period only to compute Predictors
    time_window = 2000:2012,
    
    ## COVARIATES
    # Average Share of Electricity from Coal
    ...
    # Average Pre-Treat Outcome
    co2_pre_mean = mean(co2_power_pc_tons, na.rm = TRUE)) %>%
    
    
 # Take PRedictor Summaries from above and figure out how much weight to give each donor state 
  generate_weights(
    # Period Over which donor weights are optimized  
    optimization_window = 2000:2012,
    # Smaller = More Precise : Margin of Error
    margin_ipop = 0.02,
    # How many SigFigs to use
    sigf_ipop = 7,
    # Space search for optimize CA
    bound_ipop = 6) %>%
    
 # Construct Synthetic CA Using the optimized Donor Weights    
  generate_control() 
```
...
-   They overlap pretty well before 2013, making it a credible
...
-   Pre-treatment to be at 0,

-   post-treatment to be below 0 is what we would expect to view the treatment effect
...
-   Yes, CA gap stands out compared to other states bc they didn't have the same policy applied
...
1.  Where does California rank in the MSPE ratio distribution? What does this imply about the statistical significance of the estimated effect?
    ...
    **RESPONSE:**
    ...
    -   Large ratio, meaning post-treat is divergent compared to its pre-treatment
...
**RESPONSE: The optimizer had a hard time matching sites, that matched all four variables.**

2.  How does the synthetic control compare to the simple donor pool average?

**RESPONSE: They Synthetic Ca is much closer to CA, rather than the donor sample.**
...
**RESPONSE: California's cap-and-trade program on per capita CO2 emissions are effective compared to the post treatment of synthetic CA showing a larger gap showing a evidence worked**
...
**RESPONSE: strong pre-treatment fit, could be spillover, we did see a favorable placebo comparison.**
...
**RESPONSE: If donor states also adopted cap and trade would not be an apple to apple comparison**
...
**RESPONSE: included CO2 pre-treatment average to get better matching , we are calculating synthetic information rather than comparing actual matching**
