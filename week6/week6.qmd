...
- Changes in environments, by using public transportation. 
- People are typically more sick during winter (seasons), people burn wood causing more pm2.5 to increase during winter. 
- Wildfire events, can generate more pollution (pm2.5), causing more asthma.
...
```{r}
options(noaakey = "kTKcaozuEGonZJIePaAxyRedjzWLFMfK")
```
...
trends_df <- trends$interest_over_time %>% 
    # hits are how many people search it up
    select(date, hits) %>% 
    mutate(
        date= as.Date(date),
...
- A total of 53 observations, very fragile to small sample size. This will hinder our statistics. Not represent the whole population only a small sample, inaccurate information for the whole population.
:::
...
ggplot(data_weekly_joined, aes(x= pm25_mean, y= asthma_index)) +
    ...
    labs(
        xlab = "Weekly Mean PM 2.5 (AQI)",
        ylab = "Asthma Inhaler Search Index",
...
ggplot(data_weekly_joined, aes(x= precip_total, y= pm25_mean)) +
    ...
    geom_smooth(method = "lm", se=TRUE, color = "dodgerblue", linewidth =1) +
    labs(
        xlab = "Weekly Total Precipitation (Inches)",
        ylab = "Weekly PM 2.5",
...
```{r}
m1_ols <- lm(asthma_index ~ pm25_mean, 
...
**RESPONSE:**
- The coefficient and SE increased. The role of the confounders shows seasonal patterns between asthma patterns. 
- The OLS estimate may still be biased because of the small sample size that we are estimating on. The trends in the data could be changed to weeks rather than the month pattern, could also exclude time when kids are out of school as it may decrease PM2.5.
...
**RESPONSE:**
- The sign on `precip_total` is a `-0.10`, it is intuitive with what we expected the rain removes PM2.5.
- The F-statistic (3.16) suggests a **weak instrument**, which can lead to biased IV estimates and unreliable inference. Low F-stat, explains the unexplained variance.
...
# Not correct for inference for the unexplained variance
```
...
**RESPONSE:**
- If there is more precipitation, there could be a mold growth, spreading the PM2.5
...
**RESPONSE:**
- Use the actual asthma data rather than estimations, aswell as a larger dataset, and looking at multiple cities instead of one. 
...
**RESPONSE:**
- wildfire smoke, wind and wind direction - better correlation between these variables and asthma index
- Our mechanism don't pass the test
- Relevance, weak instrument, 
- Independence/ Exogeniety - Rainfall should not be correlated with Asthma. Weather would be a better correlated, wildfires
...
**RESPONSE:**
Monotenicity - instruments shifts endogenous variable. Where precipitation is decreasing the asthma index
- 