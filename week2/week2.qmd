...
```
```
- Intercept(77.91): Without any treatment, we expect the number of treat piles to be about (~78) on average.

- Treatment (-24.77): The treatment reduced the average number of trash piles by aproximately 25 piles. 
```
...
Treatment: The model estimate that the treatment reduced waste burning is 32% in treated nieghborhoods.
```
(exp(-0.38)-1)*100 = -31.61 (Automatically impled, no need for negative sign.)
```
...
```
- Take `log` with exp to interpret

Intercept: On average, there are approximately 74% waste piles in the control group/ untreated nieghboorhoods.

Treatment: The model estimates a reduction of 34% waste piles in the treatment groups
...
m2_change = (exp(-0.38)-1) * 100    # Change in POIS #-31.6
...
- pre_count_avg only reduced by a count of 10, before the competition started. Could have other influences
:::
...
```
- Our model estimates an average of 1.6%  `> exp(0.47) = 1.599`
- In the treatment neighborhood, we see a reduction of 24% `> exp(-0.27)`
-  one unit change in  pre-count-avg
...
export_summs(m3_log, m4_control,
             ...
             statistics = "none")
```
- Differences in tables: A recent reduction with a log ols regression with a control variable. This is an attempt to reduce bias.
